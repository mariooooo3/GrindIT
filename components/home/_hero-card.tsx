...
const PERIOD_TIPS: Record<string, string> = {
  week: "Past 7 days of commits",
  month: "Past 30 days of commits",
  year: "Past 365 days of commits",
  alltime: "Your full GitHub history",
};
const TONE_TIPS: Record<string, string> = {
  funny: "Lighthearted & humorous",
  brutal: "Honest & brutally direct",
              ...
              <div className="flex gap-2">
                ...
                <div className="group relative">
                  <button onClick={handleGenerate} disabled={loading || !manualUsername.trim() || !usernameValid}
                    className="flex cursor-pointer items-center gap-2 rounded-2xl px-5 py-2 text-[13px] font-semibold text-white transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-40 whitespace-nowrap"
                    style={{
          ...
          {PERIODS.map(({ label, value, requiresAuth }) => {
            ...
            return (
              <button key={value}
                ...
                className={`group relative ${pillBase} inline-flex items-center justify-center gap-1 ${!locked && periodType === value ? pillOn : ""} ${locked ? "opacity-35 cursor-not-allowed" : !locked && periodType !== value ? pillOff : ""}`}>
        ...
        <div className="flex flex-wrap items-center justify-center gap-1.5">
          ...
          {TONES.map(({ label, value, icon: Icon }) => (
            <button key={value} onClick={() => setTone(value)}
              className={`group relative ${pillBase} inline-flex items-center gap-1.5 ${tone === value ? pillOn : pillOff}`}>
              <Icon size={12} />
              {label}
              <span className="pointer-events-none absolute left-1/2 top-full z-50 mt-1.5 -translate-x-1/2 rounded-full border border-white/10 bg-black/80 px-2.5 py-0.5 text-[9px] font-medium text-white/55 opacity-0 whitespace-nowrap transition-opacity duration-150 group-hover:opacity-100" style={{ backdropFilter: "blur(8px)" }}>
                {TONE_TIPS[value]}
