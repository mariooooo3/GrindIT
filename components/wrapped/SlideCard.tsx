  ...
  function SlideCard({ accentColor, className = "", compact = false, sizeStyle, children }, ref) {
    return (
      <div
        ...
        className={`relative flex flex-col overflow-hidden rounded-3xl [&::-webkit-scrollbar]:hidden mx-auto w-[88%] flex-1 min-h-0 ${compact ? "max-h-[var(--card-max-h-compact)]" : "max-h-[var(--card-max-h)]"} lg:max-h-none lg:w-[min(380px,92%)] lg:h-[min(580px,84vh)] lg:flex-none ${className}`}
        style={{
